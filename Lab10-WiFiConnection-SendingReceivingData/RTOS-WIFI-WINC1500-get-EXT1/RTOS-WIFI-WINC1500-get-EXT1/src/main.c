...
SemaphoreHandle_t xSemaphore_but;
...
void but_callback(void) {
  dado = !dado;
	...
	xSemaphoreGiveFromISR(xSemaphore_but, &xHigherPriorityTaskWoken);
	printf("but_callback\n");
}
...
void getget(char *g_sendBuffer, char *path){
  sprintf(g_sendBuffer, path);
}

void postpost(char *g_sendBuffer, char *path, int dado){
  sprintf(g_sendBuffer, path);
  sprintf(g_sendBuffer, "%sLED=%d", g_sendBuffer, dado);
}
...
static void task_process(void *pvParameters) {
  ...
  while(1){
    ...
    switch(state){
      ...
      if(xSemaphoreTake(xSemaphore_but,10)){
        state = POST;
      }
      else{
        state = GET;
      ...
      case GET:
      printf("STATE: GET \n");
      getget(&g_sendBuffer, &MAIN_PREFIX_BUFFER);
      ...
      printf("STATE: POST \n");
      printf("Dado: %d \n", dado);
      postpost(&g_sendBuffer, &MAIN_POST_BUFFER, dado);
      send(tcp_client_socket, g_sendBuffer, strlen((char *)g_sendBuffer), 0);
      state = ACK;
      break;
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 1000) == pdTRUE){
        ...
        printf(p_recvMsg->pu8Buffer);
        printf(STRING_EOL);  printf(STRING_LINE);
        // char led_[20] = p_recvMsg->pu8Buffer;
        char *numero;
        numero = strstr(p_recvMsg->pu8Buffer, "0");
        if (numero != NULL){
...
int main(void)
{
  ...
  xSemaphore_but = xSemaphoreCreateBinary();
	if (xSemaphore_but == NULL){
